...
//ejercicio 11 (0-5)
    @GetMapping(path = "/cantidadmayorSeller")
    public ResponseEntity<ResponseDTO>getcantidadmayorSeller(@PathVariable int quantity){
        List<Store>StoreL = new ArrayList<>();
        for (Sale sales : saleService.getSales()){
            if (saleService.getTotalSalesByStore(sales.getStore().getCode())>quantity){

                StoreL.add(sales.getStore());
            }
        }
        return new
                ResponseEntity<>(new ResponseDTO(200,StoreL,null),HttpStatus.OK);
